...
A job that points the compiler at a cache file resolves the compiler-input
fingerprint first, in a setup step that uses the
...
that entry is keyed exactly on the binary's sources and restored from no
prefix, where a compiled byte found under an older fingerprint is merely a
miss. That is why a lane restores two caches rather than one.
...
criterion. Three checks are of that shape. The `Coverage Report` job in
`deno.yml` reads the coverage artifacts of every lane in its own run, the `Test
Topology Store Check` job reads those lanes' records, and the CFC Property
Suite's audit step reads the corpus the suite in the step before it has just
written. A gate comparing against a base ref is not this:
