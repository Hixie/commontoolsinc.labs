...
`deno-test`, in a `"browser-test"` entry of their own, and its `test` task
names both: `packages/static/deno.jsonc` and `packages/ui/deno.jsonc` are the
worked examples. The coverage gate reads `deno-test` as the package's
Deno-only half, and the package's measured set holds that half's tests and
never the browser ones, so a browser test added to `browser-test` leaves the
package's gate as it was. A `test` task that joins `deno test` to the
browser run with `&&` in one string gives the topology no files to point at.
`tasks/test-topology/unit.ts` refuses to load such a member unless
`RUNS_WHOLE` lists it with its reason.
