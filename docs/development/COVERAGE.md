...
The baselines live in the manifest, and "How a `main` figure reaches the
manifest" below says how they get there. The gate walks them for the newest one
the branch contains. A checkout too shallow to answer "does this branch contain
that commit" reports every set as having no baseline, which turns the gate off
without failing anything, so `Status` checks out at full depth.
...
passes over such measurements. `tasks/coverage-records.ts` is the writer.

The Coverage Report job writes them, with `tasks/coverage-report.ts`, from
the reports the full run's lanes wrote. It runs only for a push, and ships
them under the artifact name `coverage`, which is `COVERAGE_ARTIFACT` in
`@commonfabric/test-support/records`. It runs once the lanes whose coverage
it scores have finished, so a run that failed elsewhere, in a deployment say,
still has its figures read. The relay names the object after
...
warm run never reaches and so lowers that run's uncovered count. A lane records
which it found. A lane whose batches open the cache checks whether the file
exists before its first batch runs, because the first pattern a batch compiles
writes the file whatever the cache held, and writes `cold` or `warm` to
`compile-cache-state.txt` at the top of the directory its coverage artifact
holds. A file that exists reads as warm, which is sound only while the workflow
keys the cache, and every restore key for it, on the compiler fingerprint: a
file restored from a run of another compiler holds no bytes this one can use,
and would read as warm all the same. `tasks/coverage-report.ts` reads every such
...
A measured set's figure is not affected, so the gate is not either: a set is one
suite's units over one member's lines, only the unit suites measure one, and a
unit suite never opens the compile cache, whose environment reaches only the
batches of a suite that asks for it.
...
### How a `main` figure reaches the manifest

The gate reads no run listing and downloads no artifact at the barrier; it
compares against figures the newest manifest already holds. The full run on
`main` writes each measured set's count into the record store, as "Coverage
figures in the record store" above describes, and the test-selection publisher
collects them from the objects it folds, each against the commit it was
measured at, for `LOCAL_COVERAGE_BASELINE_DAYS`.
`tasks/test-selection/baselines.ts` is where that happens.

Nothing there fails a publish. A run with no measured-set figures contributes
none. What that costs is a set with no baseline, which the gate reports rather
than fails.

### When a set is not gated

A pull request is not failed for a baseline that does not exist, so a gate that
compared nothing must not look like one that found nothing. The job summary
gives every set the change reached one outcome, and says which:

- **no rise**, **rose**, or **rose, accepted** — the set was compared.
- **no baseline on `main` yet** — no `main` commit the branch contains has a
  figure in the manifest. A later run gates it once one does; updating the
  branch gives the next run a newer commit to merge.
- **no lane reported one** — the lanes ran the set's units and wrote no report
  for it.
- **the cap left this set unforced** — the change reached more sets than
  `LOCAL_COVERAGE_MAX_SETS`, so none was forced into the lanes.
- **the reports name no line of this member** — a report arrived and measured
  nothing, which is not a measurement of zero.
- **the run has a failing test** — coverage measured through a failure says
  nothing about whether the change was tested, and the failure is the thing to
  fix.
