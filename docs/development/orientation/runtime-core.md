...
| `link-utils.ts`, `link-types.ts`, `sigil-types.ts` | The link system: the serialized `SigilLink`, the in-memory `NormalizedLink`/`NormalizedFullLink`, and the legacy `$alias` binding (`AliasBinding`), which appears in saved patterns but is deliberately not a link. |
