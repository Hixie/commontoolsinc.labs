...
| `MIN_CORRECTION_SPAN_SECONDS` | 23 | seconds | derived | Nothing edits this. It is a tenth of `LANE_BUDGET_SECONDS`, measured as the widest gap between the time two batches' own tests took, so it follows the lane bound. A suite's real slope going unbelieved for too long is a case for `MIN_CORRECTION_SAMPLES` instead. |
