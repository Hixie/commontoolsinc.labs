...
| The vintage replay | `pattern-vintage` | One per committed fixture, and one more for the replay as a whole | Nothing to reach: the task takes no way of running part of itself. |
...
Five rows hold more than one identity and offer nothing finer to reach: a
member that runs whole, a member's browser half, the reload directory,
the vintage replay, and a `fuse-exec.sh` section. Those are what `whole`
names, and the last column above says why each of them is there.
...
charge it for everything whenever anything in it was picked. One half of
that goes and the other moves.

Nothing is left to charge. `granularity` was a coarse way of saying that
running one thing costs you its neighbors, and
...
in it, which is exactly what the `"whole"` value meant, and it falls out
of the cost model rather than being a case in the packer.

What is left to declare moves one level down, to `Suite.whole`. Whether
...
and the packer's projection is short by what the rest of the unit took.
So the declaration is checked rather than trusted — every unit outside
