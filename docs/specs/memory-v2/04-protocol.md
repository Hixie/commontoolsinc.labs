...
`commit`, `query`, `query.cursor`, `db`, `db.tables`, and a `hello`
message's `flags`. Both sides refuse anything else there, and the refusal
is the one a string or a number in that position gets.
