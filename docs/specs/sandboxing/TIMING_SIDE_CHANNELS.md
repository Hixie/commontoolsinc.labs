...
3. **Close W3 channels 4–6.** The event-path input-cadence channel is closed;
   ...
   - **Channels 4 and 5 — cell-flip notification shaping (plan B).** Routing
     ...
     - **Source wiring — DONE for `$value` and server push.** The single hook is
       ...
       key is the pattern instance plus the input class — `${pieceId}|input` for
       renderer keystroke writes, `${pieceId}|push` for server pushes (see the
       interactive/passive split below); the item key is reader+cell. The reader
       ...
       reactivity is untouched. Two signals land here: `$value` renderer
       ...
       `notification.source`; server push is `notification.type` `pull`/`integrate`
       (external origin, no source). The reader `pieceId` is now set unconditionally
     ...
     - **Interactive/passive bucket split (`|input` vs `|push`) — DONE.**
       Renderer keystroke writes and server pushes go into separate per-pattern
       buckets. The reason is interaction quality: passive background traffic — a
       synced `#now` clock arriving as server pushes, another tab writing a
       shared cell — must not drain the burst allowance that keeps the user's
       own typing responsive. The honest security note (raised in security
       review): the split is a bounded widening of the per-pattern budget. A
       pattern that drives both classes at once can observe up to about two to
       three coalesced deliveries per window instead of one — the event-shaper
       bucket, the `|input` bucket, and the `|push` bucket. That is a bounded
       constant multiplier, and the per-pattern grouping that defeats the
       counting attack is retained within each class. The guarantee now rests on
       an added assumption: that server pushes (the `|push` class) are
       network-bounded and cannot be driven by a pattern at sub-second cadence —
       a pattern has no way to make the server push to it faster than real
       network traffic arrives. The `|push` bucket also has a standing worst
       case: a sustained roughly once-per-second background source keeps it
       drained, so other cross-tab updates ride the once-per-second trailing
       flush instead of arriving realtime.
