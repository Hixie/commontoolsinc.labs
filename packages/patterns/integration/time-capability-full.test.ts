...
const PER_PATTERN_TIMEOUT_MS = 30_000;
...
function withTimeout(rel: string, p: Promise<Outcome>): Promise<Outcome> {
  return Promise.race([
    p,
    new Promise<Outcome>((resolve) =>
      setTimeout(
        () =>
          resolve({
            timeCapabilityErrors: [],
            skipReason: `timeout after ${PER_PATTERN_TIMEOUT_MS}ms`,
          }),
        PER_PATTERN_TIMEOUT_MS,
      )
    ),
  ]);
}
...
describe("capability gate (W1): full pattern-set gate verification", () => {
  for (const rel of PATTERNS) {
    it(`no lift-context clock/entropy read in ${rel}`, async () => {
      const outcome = await withTimeout(rel, checkPattern(rel));
