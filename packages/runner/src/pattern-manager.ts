  ...
  private async compileViaCellCache(
    ...
    try {
      compiled = await harness.compileToRecordGraph(program, {
        fabricImports: { space },
        // The bodies returned below come either from the process byte cache
        // (modules this process already compiled + SES-verified) or from
        // `loadCompiledClosure`, an integrity-gated (`requiredIntegrity`,
        // fail-closed) read of the compiled set. On a full hit the byte cache's
        // own-process provenance / the CFC integrity label is the security
        ...
        precompiledModulesFor: async ({ entryIdentity, identities }) => {
          ...
          // storage read. Cached bytes are SES-verified when compiled, so they
          // ride the same trusted channel as the integrity-gated read.
          if (byteCache) {
