...
async function waitForSchedulerCondition(
  ...
  while (!condition() && performance.now() < deadline) {
    await runtime.idle();
    // Yield a real event-loop turn: an idle() that resolves through
    // microtasks alone would otherwise starve the task queue, and the
    // emulated server's fan-out flush — which resolves awaited commits at
    // marker coverage (CT-1950) — rides an immediate, which the fake clock
    // fires in the same pass as this timer.
