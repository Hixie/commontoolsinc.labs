...
- **Non-deterministic globals in selfcontained bodies.** **Resolved (Berni
  ...
  `safeDateNow`/`unsecureRandom` are removed and the clock/entropy intrinsics now
  throw inside a lift body at runtime (only `console.log` remains scaffolding);
  they stay structurally non-disqualifying for the transformer.
...
- **[Phase 3]** Are non-deterministic globals (`Date.now()`, `Math.random()`) OK
  ...
  `safeDateNow`, `unsecureRandom` all fine; they're injected. **Superseded:** the
  runtime now throws for the clock/entropy intrinsics inside a lift body (W1/W6
  gate), and `safeDateNow`/`unsecureRandom` are removed.
