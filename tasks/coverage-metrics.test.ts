...
Deno.test("unscoredMetricGroups names the group each unlaunched member falls in", () => {
  expect([
    ...unscoredMetricGroups([
      "./packages/shell",
  ...
  ]).toEqual(["packages/shell", "packages/connectors", "tasks"]);
});

Deno.test("unscoredMetricGroups names no group for a run that launched everything", () => {
  expect([...unscoredMetricGroups([])]).toEqual([]);
});

Deno.test("a group whose member no run launched is left out of the metrics", async () => {
  const rootDir = await Deno.makeTempDir({ prefix: "coverage-unlaunched-" });
  try {
    const writeSourceFile = async (relativePath: string): Promise<string> => {
      const fullPath = path.join(rootDir, ...relativePath.split("/"));
      await Deno.mkdir(path.dirname(fullPath), { recursive: true });
      await Deno.writeTextFile(
        fullPath,
        ["export const covered = 1;", "export const uncovered = 2;"].join("\n"),
      );
      return fullPath;
    };

    const alpha = await writeSourceFile("packages/alpha/src/mod.ts");
    await writeSourceFile("packages/beta/src/mod.ts");
    const lcov = [`SF:${alpha}`, "DA:1,1", "DA:2,0", "end_of_record"].join(
      "\n",
    );

    // With no member named, a file the report leaves out is a file no test
    // loaded, and `beta` is charged both of its lines.
    const scored = await collectCoverageDebtMetricsFromLcov({ rootDir, lcov });
    expect(scored).toEqual([
      ...
      {
        name: "coverage-debt: packages/alpha uncovered lines",
        uncoveredLines: 1,
      },
      {
        name: "coverage-debt: packages/beta uncovered lines",
        uncoveredLines: 2,
      },
    ]);

    // Naming `beta` takes its group out of the result, and the workspace total
    // with it, while the group that did run keeps the count it measured.
    const partial = await collectCoverageDebtMetricsFromLcov({
      rootDir,
      lcov,
...
Deno.test("collectCoverageDebtMetrics reads the profile directory's own record", async () => {
  ...
  try {
    ...
    // The record sits in the profile directory the conversion reads, so this
    // also states that `deno coverage` walks past it rather than failing on it.
