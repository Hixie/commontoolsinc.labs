...
describe("what a lane must run whatever the score says", () => {
  ...
  it("runs the whole measured set of a member the gate covers", () => {
    // A package scored from a run of part of its tests is scored against
    // a different quantity, so every item of its own set runs whether or
    // not the change named the file it sits in.
    const wider = suite({
      id: "workspace-unit",
      units: [
    ...
    const manifest = manifestOf([
      ...
      {
        test: { k: "unit", s: "bakery", n: "glaze > browns" },
        unit: "packages/bakery#browser-test",
      },
      {
        test: { k: "unit", s: "dairy", n: "churn > whips" },
        unit: "packages/dairy/churn.test.ts",
      },
    ]);
    const { mandatory } = census(
      [wider],
    ...
    expect(
      mandatory.get(
        testIdentityKey({ k: "unit", s: "bakery", n: "proof > rises" }),
      ),
    ).toBe("coverage-gate");
    // The browser half produces none of the coverage the gate scores, and
    // another member's tests are not the set being measured.
    expect(
      mandatory.get(
        testIdentityKey({ k: "unit", s: "bakery", n: "glaze > browns" }),
      ),
    ...
    expect(
      mandatory.get(
        testIdentityKey({ k: "unit", s: "dairy", n: "churn > whips" }),
      ),
    ).toBeUndefined();
  });
