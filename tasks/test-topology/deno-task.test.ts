...
describe("reading a task that runs the group runner", () => {
  // The runner walks a directory and runs `deno test` over its files with the
  // flags written after the separator, split by the flags each file needs. A
  // lane pointed at some of those files needs that directory and those flags.

  const TASK = "deno run --allow-env --allow-read " +
    '--allow-run="$(deno eval "console.log(Deno.execPath())")" ' +
    "../../tasks/run-test-groups.ts . " +
    "-- --no-check --allow-ffi";
  ...
  it("refuses a path written among the flags after the separator", () => {
    ...
    expect(
      parseTestTask(
        "deno run -A ../../tasks/run-test-groups.ts . " +
...
describe("reading the group runner's own arguments", () => {
  ...
  it("returns the directory it walks as the task's path", () => {
    expect(readGroupRunnerArguments(["test", "--"])?.paths).toEqual(["test"]);
  });
  ...
  it("refuses a second separator", () => {
    ...
    // an argument to the test modules rather than to `deno test`.
    expect(
      readGroupRunnerArguments([".", "--", "-A", "--", "x"]),
    ).toBeUndefined();
  });

  it("refuses arguments with no separator", () => {
    expect(readGroupRunnerArguments([".", "--no-check"])).toBeUndefined();
    expect(readGroupRunnerArguments([])).toBeUndefined();
  });
});
