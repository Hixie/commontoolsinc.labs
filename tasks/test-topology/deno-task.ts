...
/**
 * Every test file below a directory, the way `deno test` walks one:
 * each one joined onto the directory it was given, and sorted.
 *
 * Shared with `tasks/run-sharded-test-files.ts`, which hands the files
 * it walks to a `deno test` of its own. What that runner runs and what
 * the topology enumerates have to be the same set, or a member behind it
 * holds tests no unit claims.
 */
export async function testFilesUnder(directory: string): Promise<string[]> {
  const found: string[] = [];
  await walkTestFiles(directory, found);
  return found.sort();
}
