...
describe("running a member that cannot be handed a subset", () => {
  ...
  it("enumerates a member whose task runs the group runner", async () => {
    // The runner walks a directory and runs its files. A lane takes that
    // directory and runs the files it chose, so the member is read a file at a
    // time like any other.
